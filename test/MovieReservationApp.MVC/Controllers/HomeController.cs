...
namespace MovieReservationApp.MVC.Controllers
{
    public class HomeController : Controller
    {
        ...
        public async Task<IActionResult> GetSeat(int showTimeId)
        {
             SeatReservationGetVM data = null;
            ...
            ReservationCreateVM vm = new ReservationCreateVM(userId,showTimeId);
            try
            {
                await  crudService.Create("/reservations", vm);
            }
            catch (BadRequestException ex)
            {
                TempData["Err"] = ex.Message;
